...
BEGIN
    SELECT o.*, v.marca, v.modelo, v.tipo,
           ...
           c.rnt_dni, c.telefono AS cliente_telefono,
           u_rec.nombre AS recepcionista,
           u_mec.nombre AS mecanico
    FROM ordenes_servicio o
    INNER JOIN vehiculos v ON v.placa = o.placa_vehiculo
...
BEGIN
    SELECT f.id_factura, f.numero_factura, f.fecha_emision,
           f.total_pagar, f.metodo_pago, f.estado_activo,
           o.id_orden, v.placa,
...
BEGIN
    INSERT INTO facturas (
        ...
        subtotal_mano_obra, subtotal_repuestos, isv, total_pagar,
        id_orden, metodo_pago, estado_activo
    ) VALUES (
        p_numero_factura, NOW(),
        p_subtotal_mano_obra, p_subtotal_repuestos, p_isv, p_total_pagar,
        p_id_orden, p_metodo_pago, 1
    );
...
-- Obtener factura por ID con todos los datos relacionados
...
BEGIN
    SELECT f.*, o.fecha_ingreso, o.diagnostico_preliminar, o.costo_mano_obra,
           o.id_recepcionista, o.id_mecanico,
           v.placa, v.marca, v.modelo, v.anio, v.tipo,
           c.id_cliente, c.nombre AS cliente_nombre, c.rnt_dni, c.telefono AS cliente_telefono,
           u_rec.nombre AS recepcionista_nombre,
...
-- Obtener repuestos de una orden para la factura
DROP PROCEDURE IF EXISTS `sp_obtener_repuestos_por_orden`;
DELIMITER //
CREATE PROCEDURE `sp_obtener_repuestos_por_orden`(
    IN p_id_orden INT
)
BEGIN
    SELECT d.id_detalle, d.cantidad, d.precio_unitario_historico,
           (d.cantidad * d.precio_unitario_historico) AS total_linea,
           r.nombre AS repuesto_nombre, r.unidad_medida
    FROM detalles_repuestos_orden d
    INNER JOIN repuestos r ON r.id_repuesto = d.id_repuesto
    WHERE d.id_orden = p_id_orden;
END//
DELIMITER ;

-- Obtener órdenes ENTREGADO que aún no tienen factura
...
BEGIN
    SELECT CONCAT('FAC-', YEAR(NOW()), '-',
           LPAD(COALESCE(MAX(CAST(SUBSTRING_INDEX(numero_factura, '-', -1) AS UNSIGNED)), 0) + 1, 4, '0'))
           AS nuevo_numero
    FROM facturas
